...
def main():
    try:# エラーを無視する【課題2-9】
        ...
        while True:
            ...
            # ページ終了まで繰り返し取得
            name_ = driver.find_elements_by_css_selector(".cassetteRecruit__name") # cssセレクタを使った指定に変更
            info_ = driver.find_elements_by_css_selector(".cassetteRecruit__copy") # 項目2つ目 【課題2-2】
            date_ = driver.find_elements_by_css_selector(".cassetteRecruit__updateDate") # 項目3つ目 【課題2-2】
            
            # 1ページ分繰り返し
            # print(len(name_))
            for name in name_:
                exp_list.append(name.text)    

            for inf in info_:
                exp_list2.append(inf.text)

            for d in date_:
                exp_list3.append(d.text.replace("情報更新日： ",""))
        ...
        exp_df["社名"] = exp_list
        exp_df["仕事詳細"]=exp_list2
        exp_df["情報更新日"]=exp_list3

        #データをcsvファイルに出力 【課題2-5】
        exp_df.to_csv("kadai2_「" + search_keyword +"」+.csv",encoding='utf-8-sig')
